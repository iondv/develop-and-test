...
const modreq = function(props) {
  ...
  describe('Checking crud service', function() {
    ...
    describe('GET', function () {
        ...
        beforeAll(async function () {
          try {
            await request(modreq({
              ...
              uri: `${serverURL}/rest/crud/class_text@develop-and-test/`, headers: {
      ...
      describe('# getting a collections with the eager loading of the "table" property', function () {
        beforeAll(async function () {
          await request(modreq({
            method: 'POST', resolveWithFullResponse: true, json: true,
        ...
        describe('# getting an item', function () {
          ...
          it('check if the response contains three collRefCatalog objects', function () {
            expect(res.body).toBeTruthy();
            expect(res.body.table).toBeTruthy();
            expect(res.body.table.length).toEqual(3);
            res.body.table.forEach((item) => {
              expect(typeof item).toEqual('object');
              expect(item.__class).toEqual('collRefCatalog@develop-and-test');
            });
          });
        });
        describe('# getting a list', function () {
          let res;
          let req = modreq({uri: `${serverURL}/rest/crud/classColl@develop-and-test?_eager[]=table`});
          it('making the request, statusCode has to be 200', async function () {
            res = await request(req);
            expect(res.statusCode).toEqual(200);
          });
          it('check if the response contains collRefCatalog objects', function () {
            let collectionFound = false;
            expect(res.body).toBeTruthy();
            expect(res.body).toBeInstanceOf(Array);
            for (const collection of res.body) {
              if (collection.table && (collection.table.length > 0)) {
                collectionFound = true;
                collection.table.forEach((item) => {
                  expect(typeof item).toEqual('object');
                  expect(item.__class).toEqual('collRefCatalog@develop-and-test');
                });
              }
            }
            expect(collectionFound).toBeTruthy();
          });
        });
      ...
      describe('HEAD', function () {
        describe('# checking if an object is present (HEAD)', function () {
          ...
          it('check if the response does not contain objects', function () {
            // expect(res.body.length).toBeGreaterThan(0);
            // expect(res.body[0].__class).toEqual('class_text@develop-and-test');
      ...
      describe('PATCH', function () {
        describe('# updating an object (PATCH)', function () {
          ...
          it('check if the response contains the sent text object', async function () {
            ...
            expect(object.id).toEqual(reqId);
            expect(object.text_text).toEqual(`${tempText}1`);
            expect(object.text_multilinetext).toEqual(`${tempText}2`);
            expect(object.text_formattext).toEqual(`${tempText}3`);
          });
        });
        ...
        describe('# updating an inexistent object (PATCH)', function () {
          let res;
          const tempText = cryptoRandom(24).toString('hex');
          const reqId = cryptoRandom(24).toString('hex');
          let req = modreq({
